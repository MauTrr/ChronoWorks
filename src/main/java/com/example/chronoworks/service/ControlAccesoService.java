...
public class ControlAccesoService {
    ...
    public RespuestaRegistroDTO registrarEntrada(RegistroEntradaDTO dto) {
        Empleado empleado = empleadoRepository.findById(dto.getIdEmpleado()).orElseThrow(() -> new ResourceNotFoundException("Empleado no encontrado."));
    ...
    public RespuestaRegistroDTO registrarSalida(RegistroSalidaDTO dto) {
        Empleado empleado = empleadoRepository.findById(dto.getIdEmpleado()).orElseThrow(() -> new ResourceNotFoundException("Empleado no encontrado"));
        ControlAcceso registroAbierto = controlAccesoRepository.findFirstByEmpleadoAndHoraSalidaIsNullOrderByHoraEntradaDesc(empleado).orElseThrow(() -> new BadRequestException("No hay registro de entrada abierto."));
    ...
    public RespuestaRegistroDTO actualizarObservacion(Integer idControl, String nuevaObservacionEntrada, String nuevaObservacionSalida) {
        ControlAcceso controlAcceso = controlAccesoRepository.findById(idControl).orElseThrow(() -> new ResourceNotFoundException("Registro de control no encontrado."));
    ...
    public Map<String, Object> obtenerEstadoActual(Integer idEmpleado) {
        Empleado empleado = empleadoRepository.findById(idEmpleado).orElseThrow(() -> new ResourceNotFoundException("Empleado no encontrado."));
    ...
    private RespuestaRegistroDTO mapToRespuestaRegistroDTO(ControlAcceso controlAcceso) {
        ...
        return RespuestaRegistroDTO.builder().idControl(controlAcceso.getIdControl()).idEmpleado(controlAcceso.getEmpleado() != null ? controlAcceso.getEmpleado().getIdEmpleado() : null).nombre(nombreCompletoEmpleado).horaEntrada(controlAcceso.getHoraEntrada()).horaSalida(controlAcceso.getHoraSalida()).observacionEntrada(controlAcceso.getObservacionEntrada()).observacionSalida(controlAcceso.getObservacionSalida()).fecha(controlAcceso.getFecha()).build();
    }

    public List<RespuestaRegistroDTO> listarTodos() {
        List<ControlAcceso> registros = controlAccesoRepository.findAll();
        return registros.stream().map(this::mapToRespuestaRegistroDTO).toList();
    }
}